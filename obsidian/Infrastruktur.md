...
> siehe Abschnitt "Webapp-Router" oben): kein Host-Verzeichnis anzulegen, `docker/traefik/
> dynamic.yml` liegt im Repo und wird mitgemountet. `docker compose up -d --build` reicht,
> danach kurz verifizieren dass die Seite normal erreichbar ist.
...
### portal-Subdomain für Login (erledigt -- Setup-Referenz, ursprünglich 2026-07-15)
Anmelden-Button verlinkt auf `portal.stromfueralle.at/portal/login`, SSL-Terminierung auf
10.0.0.144 längst eingerichtet und seither vielfach live getestet. Setup-Schritte (für einen
Neuaufbau/eine weitere Subdomain) bleiben als Referenz in `CLAUDE.md` erhalten.
...
durchkommen (der externe Proxy reicht sie beim `proxy_pass` einfach weiter) -- widerlegt meine
vorherige Annahme, HSTS (F-01) müsse extern ergänzt werden. `Strict-Transport-Security:
max-age=31536000` steht jetzt stattdessen im Repo, bewusst ohne `includeSubDomains`/`preload`
(erst nach Bereinigung von F-04 sinnvoll). Details: `CLAUDE.md`.
